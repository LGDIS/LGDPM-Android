  ...
  describe "download_callback" do
    context "ダウンロード正常終了の場合" do
      it "APPLICマスタがロードされ、メニュー画面に遷移すること" do
        ...
        @controller.serve(@application, nil, SpecHelper.create_request("GET /Master/download_callback", "status" => "ok", "rho_callback" => true), {})
      end
    end
    context "ダウンロードエラーの場合" do
      it "エラー画面に遷移すること" do
        WebView.should_receive(:navigate).with("/app/Master/show_error")
        @controller.serve(@application, nil, SpecHelper.create_request("GET /Master/download_callback", "status" => "ng", "rho_callback" => true), {})
      end
